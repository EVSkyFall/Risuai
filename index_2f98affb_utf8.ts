...
// --- Per-chat generation tracking (Phase 2) ---
...
function registerChatJob(charId: number, chatPage: number, abortController: AbortController): string {
    ...
    doingChat.set(true)  // backward compat: at least one job is active
    return key
}
...
function unregisterChatJob(key: string) {
    ...
    // Only set doingChat to false if no jobs remain
...
export async function sendChat(chatProcessIndex = -1, arg: {
    ...
    function throwError(error: string) {
        ...
        if (DBState.db.inlayErrorResponse) {
            const chatMessages = getChatRef().message
    ...
    let isDoing = isChatGenerating(get(selectedCharID), DBState.db.characters[get(selectedCharID)]?.chatPage ?? 0)
    ...
    // Register per-chat job instead of global doingChat.set(true)
    ...
    // --- Localized DB accessors for parallelization (Phase 1) ---
    // charRef/chatRef avoid repeating DBState.db.characters[selectedChar].chats[selectedChat]
    // and make it easier to later pass these as context parameters.
    const charRef = DBState.db.characters[selectedChar]
    const getChatRef = () => charRef.chats[selectedChat]
    // --- End localized accessors ---
    getChatRef().message = getChatRef().message.map((v) => {
    ...
    if (nowChatroom.supaMemory && (DBState.db.supaModelType !== 'none' || DBState.db.hanuraiEnable || DBState.db.hypav2 || DBState.db.hypaV3)) {
        ...
        if (DBState.db.hanuraiEnable) {
            ...
            getChatRef().hypaV2Data = currentChat.hypaV2Data

            currentChat = getChatRef();
            ...
            if (sp.error) {
                ...
                if (sp.memory) {
                    ...
                    getChatRef().hypaV3Data = currentChat.hypaV3Data
                }
                console.log(sp)
                throwError(sp.error)
                return false
            ...
            getChatRef().supaMemoryData = currentChat.supaMemoryData
    ...
    if (req.type === 'fail') {
        ...
        let msgIndex = getChatRef().message.length
        let prefix = ''
        if (arg.continue) {
            msgIndex -= 1
            prefix = getChatRef().message[msgIndex].data
        }
        else {
            getChatRef().message.push({
                role: 'char',
                data: "",
                saying: currentChar.chaId,
                time: Date.now(),
        ...
        if (isPluginStream && isNodeServer) {
            ...
            // Persist current character data directly to storage
            // (bypasses BroadcastChannel that saveDb() uses, avoiding tab deactivation)
            try {
        ...
        // Note: streaming context (charIndex/chatPage) is now tracked by
        // sendRawToServer() ??addActiveJob() in __risu_active_server_jobs
        let lastResponseChunk: { [key: string]: string } = {}
        let lastScriptProcessTime = 0
        const SCRIPT_THROTTLE_MS = 150
        let chunksSinceYield = 0
        let pendingScriptProcess = false
        ...
        while (abortSignal.aborted === false) {
            ...
            if (readed.value) {
                ...
                if (now - lastScriptProcessTime >= SCRIPT_THROTTLE_MS) {
                    // Run full script processing (regex, Lua, CBS) at throttled intervals
                    let result2 = await processScriptFull(nowChatroom, reformatContent(prefix + result), 'editoutput', msgIndex)
                    getChatRef().message[msgIndex].data = result2.data
                    emoChanged = result2.emoChanged
                    lastScriptProcessTime = now
                    pendingScriptProcess = false
                } else {
                    // Between throttle intervals, show raw text for immediate feedback
                    getChatRef().message[msgIndex].data = reformatContent(prefix + result)
                    pendingScriptProcess = true
                }
                charRef.reloadKeys += 1

                // Yield event loop periodically to prevent UI freeze
                chunksSinceYield++
            ...
            if (readed.done) {
                // Final script processing to ensure correctness on stream end
                if (pendingScriptProcess && result) {
                    let result2 = await processScriptFull(nowChatroom, reformatContent(prefix + result), 'editoutput', msgIndex)
                    getChatRef().message[msgIndex].data = result2.data
                    emoChanged = result2.emoChanged
                }
                getChatRef().isStreaming = false
                charRef.reloadKeys += 1
        ...
        // Clean up plugin stream tracking ONLY on successful completion
        // On abort/refresh, localStorage key must survive for initServerChatRecovery
        if (isPluginStream && isNodeServer && streamCompleted) {
            localStorage.removeItem('__risu_active_plugin_stream')
            // Clean up server-side recovery file (server always saves it now)
        ...
        charRef.chats[selectedChat] = runCurrentChatFunction(getChatRef())
        currentChat = getChatRef()
        ...
        charRef.chats[selectedChat] = currentChat
        if (inlayr.promise) {
        ...
        for (let i = 0; i < msgs.length; i++) {
            ...
            if (i === 0 && arg.continue) {
                msgIndex -= 1
                let beforeChat = getChatRef().message[msgIndex]
            ...
            if (i === 0 && arg.continue) {
                ...
                if (inlayResult.promise) {
                    ...
                    getChatRef().message[msgIndex].data = p
                }
            }
            else if (i === 0) {
                getChatRef().message.push({
                ...
                const ind = getChatRef().message.length - 1
                if (inlayResult.promise) {
                    const p = await inlayResult.promise
                    getChatRef().message[ind].data = p
                }
        ...
        const triggerResult = await runTrigger(currentChar, 'output', { chat: currentChat })
        if (triggerResult && triggerResult.chat) {
            charRef.chats[selectedChat] = triggerResult.chat
        }
        if (triggerResult && triggerResult.sendAIprompt) {
            resendChat = true
        }
    ...
    if (igp) {
        ...
        const chatMsgs = getChatRef().message
    ...
    if (resendChat) {
        ...
        const lastMessageIndex = getChatRef().message.length - 1
        if (lastMessageIndex >= 0 && getChatRef().message[lastMessageIndex].generationInfo) {
            getChatRef().message[lastMessageIndex].generationInfo = generationInfo
        }
