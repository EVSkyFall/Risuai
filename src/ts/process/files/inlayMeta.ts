...
class NodeInlayMetaStorage {
    ...
    async entries(): Promise<[string, InlayAssetMeta][]> {
        ...
        const metaKeys = allKeys.filter(k => k.startsWith(INLAY_META_PREFIX))
        const entries: [string, InlayAssetMeta][] = []
        const ids = metaKeys.map((decodedKey) => decodedKey.replace(INLAY_META_PREFIX, ''))
        const batchSize = 200
...
export function buildInlayMeta(existingMeta?: InlayAssetMeta | null): InlayAssetMeta {
    ...
    const currentChatId = typeof (currentChat as any)?.id === 'string' ? (currentChat as any).id : undefined
